    ...
    let bottom_height = match bottom_panel {
        BottomPanel::Input => {
            let input_lines = if inner_input_width == 0 || app.input.is_empty() {
                ...
                if last_col == 0 && last_row > 0 { last_row } else { last_row + 1 }
            };
        ...
        BottomPanel::Question => {
            let opt_count = app.pending_question.as_ref().map(|q| q.options.len()).unwrap_or(0);
...
fn draw_header(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header = Paragraph::new(Line::from(vec![
        Span::styled(model_info, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
...
fn draw_messages(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    for msg in &app.messages {
        ...
        if content_lines.is_empty() || (content_lines.len() == 1 && content_lines[0].is_empty()) {
            ...
            let cursor = if msg.streaming && content_lines.len() == 1 { "\u{2588}" } else { "" };
            lines.push(Line::from(vec![
                Span::styled(prefix, prefix_style),
    ...
    let total: u16 = lines
        .iter()
        .map(|line| {
            if inner_width == 0 { return 1u16; }
            let display_width: usize = line.spans.iter()
                .map(|s| UnicodeWidthStr::width(s.content.as_ref()))
                .sum();
            1u16.max(display_width.saturating_add(inner_width - 1)
                .checked_div(inner_width).unwrap_or(1) as u16)
        })
        .sum();
    ...
    let scroll = if app.scroll_pinned { max_scroll } else {
        max_scroll.saturating_sub(app.scroll_offset)
    };

    let title = if !app.scroll_pinned {
        let position = max_scroll.saturating_sub(scroll);
        format!("Conversation [scroll: {}/{}]", position.min(max_scroll), max_scroll)
    } else {
...
fn draw_input(frame: &mut Frame, app: &App, area: Rect) {
    let input = Paragraph::new(app.input.as_str())
        .block(Block::default().borders(Borders::ALL).title(
            if app.agent_busy { "Waiting..." } else { "Input" },
        ))
        .wrap(Wrap { trim: false });
...
fn draw_status_bar(frame: &mut Frame, app: &App, area: Rect) {
    let usage = app.token_usage.as_ref().map_or(String::new(), |u| {
        format!(" | Tokens: {}in/{}out | ${:.4}", u.input_tokens, u.output_tokens, u.cost_usd)
    });
